...
import { CalendarEventForm, ICalendarEventForm } from "../../../models/Calendar/CalendarEventForm";
...
const CalendarContent: FC<any> = (): JSX.Element => {
    ...
    const [valueTab, setValueTab] = useState(TabManagement.EventInfo);
    ...
    // const getCurrentUserLogin = (): void => {
    //     UserService.getUserById().then(result => {
    //         seCurrentUserData({...result.data});
    //     });
    // };

    const onShowAddEventDialog = (): void => {
        setShowAddEventDialog(true);
        setValueTab(TabManagement.EventInfo);
    };
    ...
    // const onShowConfirmDialog = (): void => {
    //     const isDirtyForm = checkFormIsDirty();
    ...
    const checkPermissionAction = (): boolean => {
        // return isExistRoles([E_ROLES.Admin, E_ROLES.CalendarAdmin], currentUserData?.listRoleShortName);
        return false;
    };
    ...
    const getAllEvent = (date): void => {
        if (rangeCalendarView && calendarApi) {
            ...
            // CalendarEventService.getAllEvent(date)
            //     .then(result => {
    ...
    const getEventById = (id: string): void => {
        ...
        // const param : IDetailCalendarView = {
        //     id: id,
        //     minutesOffset: (new Date()).getTimezoneOffset()
        // };
        // TaskService.getById(param, id)
        //     .then(res => {
    ...
    const getGuestByEvent = (): void => {
        // setSubmitting(true);

        // TaskService.getGuestByEvent(eventId)
        //     .then(res => {
        //         const data = res.data;

        //         seGuestDetail(data);
        //         setUserAdditions(data?.userAdditions);
        //     })
        //     .finally(() => {
        //         setSubmitting(false);
        //     });
    ...
    const handleSubmitDeleteEvent = (): void => {
        // setSubmitting(true);
        // TaskService.deleteById(currentEvent.id)
        //     .then((result) => {
        //         if (result.statusCode === HttpStatusCode.Ok) {
        //             getAllEvent(rangeCalendarView);
        //             setShowDetailEventDialog(false);
    ...
    const handleSendInviteGuest = (): void => {
        setSubmitting(true);
        // TaskService.sendInviteGuest(currentEvent.id)
        //     .then((result) => {
        //         if (result.statusCode === HttpStatusCode.Ok) {
        //             getAllEvent(rangeCalendarView);
        //         }
    ...
    const dayClickCallback = (data: DateClickArg): void => {
        console.log('dayClickCallback', data);
    ...
    // const checkFormIsDirty = (): boolean => {
    //     return TabManagement.EventInfo === valueTab
    //         ? Object.keys(formCalendarEvent.formState.dirtyFields).length > 0 
    //         : Object.keys(formGuest.formState.dirtyFields).length > 0;
    // };

    // const handleChange = (_, newValue: string): void => {
    //     setValueTabTemp(newValue);
    //     const isDirtyForm = checkFormIsDirty();

    //     if (isDirtyForm) {
    //         setShowConfirmChangeTabDialog(true);
    //     } else {
    //         setValueTab(newValue);

    //         if (TabManagement.EventInfo === newValue) {
    //             if (eventId) {
    ...
    const handleChangeTabDetail = (_, newValue: string): void => {
        setValueTab(newValue);
        ...
        else if (TabManagement.GuestsDetail === newValue) {
            if (eventId) {
                getGuestByEvent();
            }
        }
    ...
    const clearForm = (): void => {
        if (TabManagement.EventInfo === valueTab) {
    ...
    const submitForm = (): void => {
        ...
        if (TabManagement.EventInfo === valueTab) {
            ...
            // formGuest.handleSubmit(onSubmitGuestCalendarEvent)();
        }
    };

    const onChangeKeySearchGuest = (value: string): void => {
        setShowBtnClearText(value.trim() ? true : false);
        setKeySearchGuest(value);
    };
    
    const onClearKeySearchGuest = async (): Promise<void> => {
        setShowBtnClearText(false);
        setKeySearchGuest('');
        loadDataGuest('');
    };

    const onSubmitSearchGuest = async (): Promise<void> => {
        const valueSearch = keySearchGuest?.trim();
        loadDataGuest(valueSearch);
    };

    const loadDataGuest = (valueSearch: string): void => {
        // TaskService.searchGuestByEventDetail(eventId, valueSearch)
        //     .then(res => {
        //         const dataUserGuests = res?.data;
                    
        //         setUserAdditions(dataUserGuests || []);
        //     });
    };
                        ...
                        <BoxTabListCalender>
                            <TabListCalender onChange={handleChangeTabDetail} aria-label="lab tab">
                                ...
                                <Tab 
                                    icon={<GroupsIcon/>}
                                    iconPosition='start'
                                    // label={convertEnumValueToLabel(TabManagement.GuestsDetail)}
                                    value={TabManagement.GuestsDetail} 
                                />         
